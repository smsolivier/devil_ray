#ifndef DRAY_MEM_STREAM_H
#define DRAY_MEM_STREAM_H
...
struct Serialization<std::vector<T>>
{
  static void write(MemStream &memstream, const std::vector<T> &data)
  {
    ...
    dray::write(memstream, sz);
    for (size_t i = 0; i < sz; i++)
        dray::write(memstream, data[i]);
  }

  static void read(MemStream &memstream, std::vector<T> &data)
  {
    size_t sz;
    dray::read(memstream, sz);
    data.resize(sz);
    for ( size_t i = 0; i < sz; i++ )
        dray::read(memstream, data[i]);
  }
};
...
struct Serialization<std::list<T>>
{
  ...
  {
    dray::write(memstream, data.size());
    typename std::list<T>::const_iterator it;
    for (it = data.begin(); it != data.end(); it++)
        dray::write(memstream, *it);
  }
